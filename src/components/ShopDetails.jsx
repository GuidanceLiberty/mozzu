...
const ShopDetails = () => {
  // Inside your ShopDetail Component:
  const { id } = useParams();
  ...
  const currentProduct =
    shopPage.find((p) => p.id === Number(id)) || shopPage[0];

  // Handle Next/Prev cycling through the entire shopPage list
  const handleNext = () => {
    const currentIndex = shopPage.findIndex((p) => p.id === currentProduct.id);
    const nextIndex = (currentIndex + 1) % shopPage.length;
    navigate(`/shop-detail/${shopPage[nextIndex].id}`);
  };

  // Logic to handle Add to Cart for the backend
  const handleAddToCart = () => {
    const cartItem = {
      productId: currentProduct.id,
      name: currentProduct.name,
      price: currentProduct.newPrice || currentProduct.oldPrice,
      qty: quantity,
  ...
  const handlePrev = () => {
    const currentIndex = shopPage.findIndex((p) => p.id === currentProduct.id);
    const prevIndex = (currentIndex - 1 + shopPage.length) % shopPage.length;
    navigate(`/shop-detail/${shopPage[prevIndex].id}`);
  };
      ...
      <section className="relative w-full h-[450px] md:h-[550px] text-white overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-4 w-full">
          ...
          >
            ...
            <span className="text-xl">›</span>
            <span className="opacity-80">Shop Details</span>
          </motion.div>
        </div>
      </section>

      {/* ================= SHOP DETAILS SECTION ================= */}
      <section className="relative w-full bg-white py-12 md:py-24 px-4">
        <div className="max-w-[1400px] mx-auto">
          <div className="flex flex-col lg:flex-row gap-8 md:gap-12 lg:gap-20 items-start">
            ...
            <div className="w-full lg:w-1/2 space-y-8">
              <div className="relative aspect-square md:aspect-4/3 rounded-[30px] overflow-hidden bg-[#FEF4EB] shadow-sm">
                ...
                <button
                  onClick={handlePrev}
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/60 rounded-full flex items-center justify-center hover:bg-white transition-all cursor-pointer"
                >
                  <ChevronLeft />
                </button>
                <button
                  onClick={handleNext}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/60 rounded-full flex items-center justify-center hover:bg-white transition-all cursor-pointer"
                >
                  <ChevronRight />
                </button>
              </div>

              {/* THUMBNAILS (Cycles through all shop items) */}
              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x">
                {shopPage.map((item) => (
                  <Link
                    key={item.id}
                    to={`/shop-detail/${item.id}`}
                    className={`w-24 h-24 shrink-0 rounded-2xl border-2 transition-all p-2 flex items-center justify-center bg-[#F9F9F9] ${
                      Number(id) === item.id
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-500 font-medium">
                    {currentProduct.ratings} | (4 Reviews)
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 font-bold">
                  <span>{currentProduct.mode}</span>
                  <span className="text-red-500">{currentProduct.status}</span>
                </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center border rounded-xl h-14 bg-white">
                      <button
                        ...
                        className="w-12 h-full flex items-center justify-center hover:text-orange-500 cursor-pointer"
                      >
                        <Minus size={18} />
                      </button>
                    ...
                    <button
                      ...
                      className="flex-1 max-w-[220px] h-14 bg-orange-500 text-white rounded-xl font-bold flex items-center justify-center gap-3 shadow-lg hover:bg-orange-600 transition-all cursor-pointer"
                    >
                      <ShoppingCart size={22} /> Add To Cart
                    </button>

                    <button className="w-14 h-14 flex items-center justify-center rounded-xl bg-orange-50 text-orange-500 border border-orange-100 hover:bg-orange-500 hover:text-white transition-all cursor-pointer">
                      <Heart size={24} />
                    </button>
                  </div>
                </div>
          ...
          <div className="mt-20 border-t pt-16">
            <div className="flex gap-12 border-b mb-10 overflow-x-auto scrollbar-hide">
              {["Description", "Specification", "Reviews"].map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`pb-4 text-xl font-bold transition-all relative cursor-pointer ${activeTab === tab ? "text-orange-500" : "text-gray-400"}`}
                >
            ...
            <div className="text-gray-600 text-lg leading-relaxed max-w-4xl">
              {activeTab === "Description" && (
                <p>{currentProduct.longDescription}</p>
              )}
              {activeTab === "Specification" && (
                <p>
                  Cooking time: {currentProduct.time}. Ingredients: Organic
                  vegetables, artisan bread, chef's signature sauce.
                </p>
              )}
              {activeTab === "Reviews" && (
                <p>
                  No reviews yet for {currentProduct.name}. Be the first to
                  review!
                </p>
              )}
            </div>
